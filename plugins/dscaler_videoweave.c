...
static DEINTERLACE_METHOD *di_videoweave;

static void deinterlace_frame_di_videoweave( unsigned char *output,
                                           ...
                                           int bottom_field, int width, int height )
{
    ...
    di_videoweave->pfnAlgorithm( &Info );
}

static deinterlace_method_t videoweavemethod =
{
    DEINTERLACE_PLUGIN_API_VERSION,
    "VideoWeave (Dscaler DLL)",
    "VideoWeave",
    4,
    MM_ACCEL_X86_MMXEXT,
    0,
    ...
    0,
    0,
    deinterlace_frame_di_videoweave
};
...
#endif
{
    di_videoweave = load_dscaler_deinterlacer( "DI_VideoWeave.dll" );
    if( di_videoweave ) {
